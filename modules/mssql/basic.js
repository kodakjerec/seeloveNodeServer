...
router.post('/employeeNew', async (req, res) => {
  try {
    ...
    const queryResult = await pool.request()
      ...
      .input('TelHome', sql.NVarChar, form.TelHome)
      .input('TelMobile', sql.NVarChar, form.TelMobile)
      .input('Grade', sql.NVarChar, form.Grade)
...
router.post('/storageAddressUpdate', async (req, res) => {
  try {
    ...
    const queryResult = await pool.request()
      ...
      .input('Column', sql.VarChar, form.Column)
      .input('Row', sql.VarChar, form.Row)
      .input('Grid', sql.VarChar, form.Grid)
  ...
  try {
    ...
    for (let i=2; i <= worksheet.rowCount; i++){
      ...
      if (isUpdate) {
        ...
        .input('Direction', sql.VarChar, row.getCell(9).value)
        .input('Category1', sql.VarChar, row.getCell(10).value)
        .input('Category2', sql.VarChar, row.getCell(11).value)
        .input('Category3', sql.VarChar, row.getCell(12).value)
        .input('Length', sql.Decimal, row.getCell(13).value)
        .input('Width', sql.Decimal, row.getCell(14).value)
        .input('Height', sql.Decimal, row.getCell(15).value)
        .input('MaxQty', sql.Int, row.getCell(16).value)
        .input('AvgQty', sql.Int, row.getCell(17).value)
        .input('Status', sql.VarChar, '1')
        .input('Memo', sql.NVarChar, row.getCell(18).value)
        .input('StorageType', sql.VarChar, row.getCell(19).value)
        .input('Column', sql.VarChar, row.getCell(6).value)
        .input('Row', sql.VarChar, row.getCell(7).value)
        .input('Grid', sql.VarChar, row.getCell(8).value)
        .execute('basic_StorageAddressUpdate')
